...
model = Net()
model.load_state_dict(torch.load('model2_99%.pt'))
model.eval()
...
def ispositive_example(roi):
    ...
    return class_belongs[0],prob[0][1]
...
cv2.namedWindow('GCPDetection',cv2.WINDOW_NORMAL)
...
output_path = "/home/ivlabs/users/karthik/gcp_detection/CV-Assignment-Output-model2/"

csv_data = [['FileName','GCPLocation']]
for file in glob.glob("*.JPG"):
    # if file == "DJI_0036.JPG":
    csv_row = []
    gcp_list = []
    print(file)
    img = cv2.imread(file)
    ...
    for cnt in contours:
        ...
        if a / b <= 1.8:                                    #filter Rectangles having aspect ration more than 3:2
            ...
            if k is False:                                  #filter based on convexity
                if w*h > 36:                                  #filter minute rectangles based on area threshold
                    roi = closing[y:y+h, x:x+w]
                    roi_dilated = cv2.dilate(roi, kernel, iterations=1)
                    padded_roi = resize_roi(roi)
                    class_belongs , prob = ispositive_example(padded_roi)
                    if class_belongs == 1:
                        if prob > 0.7:
